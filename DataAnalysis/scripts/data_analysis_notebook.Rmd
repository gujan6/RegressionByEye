...
## 1. Open the data
1.1. if **our data**
Run to join all data files into a single one (do just once):
```{r}
...
write.csv(our_res, file = "./data/trend-bias-our-results.csv")
```

Open a file with all our results:
```{r}
all_results <- read.csv("./data/trend-bias-our-results.csv")
```
...
```{r}
ols1 <- lmer(Error..unsigned. ~ Sigma*Graph.Type*Type + M + (1|Participant), data=orig_all_results)
ancova <- anova(ols1)
